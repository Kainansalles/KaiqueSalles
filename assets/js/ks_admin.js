...
//            }
		...
		$('.field-input .input-date').each(function () {
                ...
                insert: {
                    ...
                    click: function () {
                        ...
                        models.each(function (e) {
                            ...
                            item_control += '</div>';
                            item_control += '<img class="objectImage" src="' + attm.url + '" width="150"/>';
                            item_control += '<input type="hidden" value="' + attm.id + '" name="post_portfolio[image_gallery][]"/>';
        ...
        $('.button.accept-volunteer').click(function () {
            var target = $(this),
            id = target.data('id'),
            parent = target.parents('tr');
            $.ajax({
                url: inFundingCfg.ajaxUrl,
                data: {action: 'acceptVolunter', id: id},
                type: "post",
                success: function (data) {
                    var rs = $.parseJSON(data);
                    if (rs.success) {
                        parent.find('td.status').text(rs.text);
